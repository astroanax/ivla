...
class ArmGripperAction(BaseModel):
    ...
    def check_arm(cls, v):
        if len(v) != 7:
            raise ValueError('[Franka] Arm action must be 1x7 list')
    ...
    def check_gripper(cls, v):
        if isinstance(v, list):
            if len(v) != 2 and len(v) != 6:
                raise ValueError('[Franka] Gripper action list must be 1x2(panda) or 1x6(robotiq)')
        elif isinstance(v, int):
            if v not in (-1, 1):
                raise ValueError('[Franka] Gripper action num must be -1 or 1')
        else:
            raise ValueError('[Franka] Gripper action type must be list or int')
...
class EEfAction(BaseModel):
    ...
    def check_position(cls, v):
        if len(v) != 3:
            raise ValueError('[Franka] EEf position must be 1*3 list')
    ...
    def check_orientation(cls, v):
        if len(v) != 4:
            raise ValueError('[Franka] EEf orientation must be 1*4 list (quaternion: (w, x, y, z))')
...
def validate_action(
    action,
) -> Tuple[Literal['joint', 'arm_gripper', 'eef'], Union[list, ArmGripperAction, EEfAction]]:
    try:
        if isinstance(action, list):
            if len(action) != 9 and len(action) != 13:
                raise ValueError('[Franka] Joint action must be 1x9(panda) or 1x13(robotiq) list')
            return 'joint', action
        ...
        raise ValueError('[Franka] Invalid action structure')

    except (ValidationError, ValueError) as e:
        print(
            '=' * 50,
            '\n[Franka] Action supports following types:\n'
            '1. joint action (List): List[float] with length 1x9(panda) or 1x13(robotiq)\n'
            '2. arm_gripper_action (Dict):\n'
            "{'arm_action': 1x7 List[float],\n"
            "'gripper_action': 1x2(panda) or 1x6(robotiq) List[float] or int -1/1 (open/close)}\n"
