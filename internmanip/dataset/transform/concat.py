...
from internmanip.dataset.schema import DatasetMetadata, StateActionMetadata
    ...
    def apply(self, data: dict) -> dict:
        ...
        if "video" in grouped_keys:
            ...
            for video_key in self.video_concat_order:
                ...
                unsqueezed_video = video_data.unsqueeze(dim=-4)  # [..., H, W, C] -> [..., 1, H, W, C]
                unsqueezed_videos.append(unsqueezed_video)
            # Concatenate along the new axis
            unsqueezed_video = torch.cat(unsqueezed_videos, dim=-4)  # [..., V, H, W, C]

            # Video
            data["video"] = unsqueezed_video # tensor
