...
from internmanip.model.basemodel.openpi0.mixture import Mixture
import torch
...
class JointModel(nn.Module):
    def __init__(self, config):
        ...
        self.num_mixture = len(config.mixture.__dataclass_fields__)
        self.cache_names = [ ]
        for field_names in list(config.mixture.__dataclass_fields__.keys()):
            if getattr(config.mixture, field_names).cache:
                self.cache_names.append(field_names)
        ...
        for field_names in list(config.mixture.__dataclass_fields__.keys()):
            ...
            # new_config = merge_dataclass(config, getattr(config.mixture, field_names))
            self.mixtures[field_names] = Mixture(config, getattr(config.mixture, field_names))
        self.mixture_names = list(config.mixture.__dataclass_fields__.keys())
