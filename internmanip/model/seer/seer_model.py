from internmanip.model.seer.vit_mae import MaskedAutoencoderViT
from internmanip.model.seer.perceiver_resampler import PerceiverResampler
from internmanip.model.seer.gpt2 import GPT2Model
from internmanip.model.basemodel import BasePolicyModel
...
class SeerModel(BasePolicyModel):
    def __init__(
        self, 
        ...
        **kwargs
        ...
        self.finetune_type = kwargs.get("finetune_type", "default")
        self.sequence_length = kwargs.get("sequence_length", 10)
        self.action_pred_steps = kwargs.get("action_pred_steps", 1)
        self.obs_pred = kwargs.get("obs_pred", False)
        self.atten_goal = kwargs.get("atten_goal", False)
        self.atten_goal_state = kwargs.get("atten_goal_state", False)
        self.atten_only_obs = kwargs.get("atten_only_obs", False)
        self.attn_robot_proprio_state = kwargs.get("attn_robot_proprio_state", False)
        self.mask_l_obs_ratio = kwargs.get("mask_l_obs_ratio", 0.0)
        self.hidden_dim = kwargs.get("hidden_dim", 384)
        self.phase = kwargs.get("phase", "evaluate")
        assert self.phase in ["pretrain", "finetune", "evaluate"]
        self.gripper_width = kwargs.get("gripper_width", False)
        self.vit_checkpoint_path = kwargs.get("vit_checkpoint_path", "./mae_pretrain_vit_base.pt")
        self.PATCH_SIZE = kwargs.get("patch_size", 16)
        self.transformer_layers = kwargs.get("transformer_layers", 12)
        self.transformer_heads = kwargs.get("transformer_heads", 12)
        ...
        self.vision_encoder = MaskedAutoencoderViT(
            patch_size=self.PATCH_SIZE, embed_dim=768, depth=12, num_heads=12,
        ...
        self.NUM_RESAMPLER_QUERY = kwargs.get("num_resampler_query", 9)
        self.perceiver_resampler = PerceiverResampler(dim=self.RESAMPLER_hidden_dim, num_latents=self.NUM_RESAMPLER_QUERY, depth=3).to(self.cuda_device)
        ...
        # obs_token
        self.NUM_OBS_TOKEN_PER_IMAGE = kwargs.get("num_obs_token_per_image", 10)
        ...
        gpt2_config.n_layer = self.transformer_layers
        gpt2_config.vocab_size = 1
        gpt2_config.n_head = self.transformer_heads
        self.transformer_backbone = GPT2Model(gpt2_config).to(self.cuda_device)
        ...
        self.NUM_MASK_TOKEN = int(kwargs.get("calvin_input_image_size", 224)**2 / self.PATCH_SIZE / self.PATCH_SIZE)  # i.e. num_patch
        ...
        # # freeze text encoder
        self.text_processor_fn = functools.partial(clip.tokenize, truncate=True)
        if os.path.exists("checkpoints/clip/ViT-B-32.pt"):
            self.clip_model, self.image_processor = clip.load("checkpoints/clip/ViT-B-32.pt", device=self.cuda_device)
        else:
            self.clip_model, self.image_processor = clip.load("ViT-B/32", device=self.cuda_device)
        
        self._init_model_type()

        if kwargs.get("resume_from_checkpoint") is not None:
            self.load_from_checkpoint(kwargs.get("resume_from_checkpoint"))
    ...
    def load_from_checkpoint(self, ckpt_path):
        ...
        model_state_dict = {k.replace('module.', '', 1) if k.startswith('module.') else k: v for k, v in model_state_dict.items()}
        self.load_state_dict(model_state_dict, False)
        self.eval()
    ...
    def forward(self, image_primary, image_wrist, state, text_token, action=None):  
        if self.training and self.phase == "pretrain":
            ...
            self.attention_mask = nn.Parameter(generate_attention_mask(
                            ...
                            mask_l_obs_ratio=self.mask_l_obs_ratio,
                            num_obs_token=this_num_obs_token,
                            action_pred_steps=self.action_pred_steps).to(self.cuda_device), 
                            requires_grad=False)
        B, S, _ = state.shape
